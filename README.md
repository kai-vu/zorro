...
- Benchmark LLM answers to documentation-focused competency questions with `python -m src.competency_llm_eval`. This script feeds the model the full non-extraction RDF graph and reports precision/recall/F1 against the SPARQL gold answers (use `--cache-only` to reuse previously stored responses).
- Alternatively, `python -m src.competency_llm_eval_subgraph` evaluates each question against a localised subgraph (default depth 1). Add `--cache-only` to avoid new LLM calls, `--depth N` to widen the extracted neighbourhood, or `--profile-subgraphs` to dump only the retrieved subgraphs (Turtle under `queries/reports/subgraph-profiles/`) and JSON telemetry for debugging. Each run records the retrieval request and triple breakdown in `queries/reports/competency_llm_subgraph_cache.json`, `queries/reports/competency_llm_subgraph_results.json`, and `queries/reports/competency-llm-subgraph-report.md` to aid monitoring.
