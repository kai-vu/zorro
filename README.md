...
- Alternatively, `python -m src.competency_llm_eval_subgraph` evaluates each question against a localised subgraph (default depth 1). Add `--cache-only` to avoid new LLM calls or `--depth N` to widen the extracted neighbourhood.
